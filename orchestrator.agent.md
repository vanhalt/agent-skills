...
If the stack is Ruby on Rails:

- Prefer Rails conventions over custom abstractions
- Distinguish between model, migration, controller, service, job, mailer, view, Stimulus controller, and system/test work
- Account for Hotwire/Turbo/Stimulus flows where relevant
- Prefer importmap-compatible frontend work unless the stack says otherwise
- Include database, test, and UI verification steps when appropriate

If the stack is unclear, ask before proceeding.
